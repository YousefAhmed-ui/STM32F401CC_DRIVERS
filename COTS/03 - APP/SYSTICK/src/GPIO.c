...
GPIO_enuErrorStatus_t gpio_SetPinValue( void * Copy_u8Port , u16 Copy_u16Pin , u8 Copy_u8PinValue )
{
    ...
    else
    {
        for(Local_u8Iterator = 0 ; Local_u8Iterator < GPIO_PIN_NUM ; Local_u8Iterator++)
       {
            ...
            if( Local_u16CurrentPin == GPIO_TRUE )
            {
                ...
                {
                    ((GPIOReg_t *)Copy_u8Port) -> BSRR |= (1<<Local_u8Iterator);
                }
                else
                {
                    ((GPIOReg_t *)Copy_u8Port) -> BSRR |= (1<<Local_u8Iterator+BSRR_RESET_OFFSET);
                } 
                //switch( Copy_u8Port )
                //{
                //    case GPIO_PORTA :
